<?php
	$inactive_selected = FALSE;
	$active_selected = FALSE;
	$delete_selected = FALSE;
	switch($dealer_array[0]->dealer_status) {
		case 'inactive':
			$inactive_selected = TRUE;
			break;
		case 'active':
			$active_selected = TRUE;
			break;
		case 'delete':
			$delete_selected = TRUE;
			break;
	}
	
	$random = rand(100,999);
?>
<div id="list_page_header">
	<h1 class="news_header">ACCOUNT INFO - UPDATE PROFILE</h1>
</div>
<div class="flashdata">
    <?php 
		if(validation_errors()) {
			echo '<div class="error_alert">' . "\n";
    		echo '<p>You have encountered some errors on this page, please review below.</p>' . "\n";
    		echo '</div>' . "\n";
		}
		if(isset($error)) {
			echo $error;
		}
		echo $this->session->flashdata('status_message');
	?>
</div>
<div class="clear" style="height:20px;"> </div>
<div id="instruction_copy">
	<p>This form allows you to enter information about your business, including your business' name, phone number, street address, website, company logo and credentials (professional organization memberships). Don't forget to click the "update profile" button at the bottom of this page to save your changes.</p>
    
    <p><b>NOTE:</b> <i>The ability to update your phone number has been disabled. Please alert VELUX if you need to change your phone number for any reason.</i></p>
</div>
<?php echo form_open_multipart('dealer-admin/account/update/' . $this->session->userdata('dealer_id')); ?>
	<input type="hidden" name="dealer_id" value="<?php echo $this->session->userdata('dealer_id'); ?>" />
    <input type="hidden" name="phone1" value="<?php echo $dealer_array[0]->phone1; ?>" />
    <?php
		if(trim($dealer_array[0]->dealer_logo) != '') {
			echo '<input type="hidden" name="current_filename_base" value="' . $dealer_array[0]->dealer_logo . '" />';
			echo '<input type="hidden" name="current_filename" value="' . $dealer_array[0]->dealer_logo . '.' . $dealer_array[0]->extension . '" />';
		}
	?>

	<label for="name">Dealer Name</label><br />
    <input type="text" name="name" class="input_text" value="<?php echo set_value('name', $dealer_array[0]->name); ?>" />
    <div class="error_message"><?php echo error_message('name'); ?> </div><br /><br />
    
    <label for="name">Account E-mail Address</label><br />
    <input type="text" class="input_text" name="email" value="<?php echo set_value('email', $dealer_array[0]->email); ?>" /><br />
    <div class="error_message"><?php echo error_message('email'); ?> </div><br /><br />
    
    <div class="content_divider"> </div>
    <h3>Contact Information</h3>
    
    <table cellpadding="0" cellspacing="0" border="0">
    	<tr valign="top">
        	<td width="450">
    
                <label for="name">Contact First Name</label><br />
                <input type="text" class="input_text" name="contact_first_name" value="<?php echo set_value('contact_first_name', $dealer_array[0]->contact_first_name); ?>" />
                <div class="error_message"><?php echo error_message('contact_first_name'); ?> </div>
            </td>
            <td>  	
                <label for="name">Contact Last Name</label><br />
                <input type="text" class="input_text" name="contact_last_name" value="<?php echo set_value('contact_last_name', $dealer_array[0]->contact_last_name); ?>" />
                <div class="error_message"><?php echo error_message('contact_last_name'); ?> </div><br /><br />
            </td>
        </tr>
        <tr valign="top">
        	<td>
                <label for="name">Address</label><br />
                <input type="text" class="input_text" name="address" value="<?php echo set_value('address', $dealer_array[0]->address); ?>" />
                <div class="error_message"><?php echo error_message('address'); ?> </div>
            </td>
            <td>
            	<label for="name">Address 2</label><br />
    			<input type="text" class="input_text" name="address2" value="<?php echo set_value('address2', $dealer_array[0]->address2); ?>" />
    			<div class="error_message"><?php echo error_message('address2'); ?> </div><br /><br />
            </td>
        </tr>
        <tr valign="top">
        	<td>
    			<label for="name">City</label><br />
    			<input type="text" class="input_text" name="city" value="<?php echo set_value('city', $dealer_array[0]->city); ?>" />
    			<div class="error_message"><?php echo error_message('city'); ?> </div>
            </td>
            <td>
                <label for="state">State</label><br />
                <select name="state" class="input_dropdown">
                    <option value="">Please choose</option>
                    <?php 
                        $state_array = get_data_array('state');
                        foreach($state_array as $key => $value) {
                            $selected = $dealer_array[0]->state == $key ? TRUE : FALSE;
                            echo '<option value="' . $key . '"' . set_select('state',$key,$selected) . '>' . $value . '</option>' . "\n";
                        }
                    ?>
                </select>
                <div class="error_message"><?php echo error_message('state'); ?> </div><br /><br />
            </td>
        </tr>
        <tr valign="top">
        	<td>
                <label for="name">ZIP</label><br />
                <input type="text" class="input_text" name="zip" value="<?php echo set_value('zip', $dealer_array[0]->zip); ?>" /><br />
                <div class="error_message"><?php echo error_message('zip'); ?> </div><br /><br />
            </td>
            <td>
            	<label for="name">Region</label><br />
                <input type="text" class="input_text" name="region" value="<?php echo set_value('region', $dealer_array[0]->region); ?>" /><br />
                <div class="error_message"><?php echo error_message('region'); ?> </div><br /><br />
    		</td>
        </tr>
        <tr valign="top">
        	<td>
                <label for="name">Phone</label><br />
                <?php
					/*
					<input type="text" class="input_text" name="phone1" value="<?php echo set_value('phone1', $dealer_array[0]->phone1); ?>" disabled /><br />
					<div class="error_message"><?php echo error_message('phone1'); ?> </div><br /><br />
					*/
					echo '<b>' . $dealer_array[0]->phone1 . '</b>';
				?>
                
            </td>
            <td>
                <label for="name">Fax</label><br />
                <input type="text" class="input_text" name="fax" value="<?php echo set_value('fax', $dealer_array[0]->fax); ?>" /><br />
                <div class="error_message"><?php echo error_message('fax'); ?> </div><br /><br />
            </td>
        </tr>
        
        <tr valign="top">
        	<td>
                <label for="name">Website URL</label><br />
                <input type="text" class="input_text" name="website" value="<?php echo set_value('website', $dealer_array[0]->website); ?>" /><br />
                <div class="error_message"><?php echo error_message('website'); ?> </div><br /><br />
            </td>
            <td>
            	<label for="microsite_url">Solar Microsite URL</label><br />
                <input type="text" class="input_text" name="microsite_url" value="<?php echo set_value('microsite_url', $dealer_array[0]->microsite_url); ?>" /><br />
                <div class="error_message"><?php echo error_message('microsite_url'); ?> </div><br /><br />
    		</td>
        </tr>
    </table>
    
    <label for="dealer_hours">Dealer Hours</label><br />
    <textarea name="dealer_hours" class="textarea_text"><?php echo set_value('dealer_hours', $dealer_array[0]->dealer_hours); ?></textarea>
    <div class="error_message"><?php echo error_message('dealer_hours'); ?> </div><br /><br />
    
    <div class="content_divider"> </div>
    <h3>Company Logo</h3>
    <p class="instruction">Logos should be one of the following formats: .jpg, .png or .gif. If your logo exceeds 115px in height, it will be resized automatically. <b>Images for upload must be less than 1.5MB in file size</b>.</p>
    
    <label for="logo">Logo</label><br />
    <input type="file" name="userfile" />
    <?php
		if(trim($dealer_array[0]->dealer_logo) != '') {
			echo '&nbsp;&nbsp;Current Logo: <a class="lightbox" href="/dealer_assets/dealer_logos/' . $dealer_array[0]->dealer_logo . '.' . $dealer_array[0]->extension . '?rand=' . $random . '">' . $dealer_array[0]->dealer_logo . '.' . $dealer_array[0]->extension . '</a>';
			echo '<br /><a href="/dealer-admin/account/delete-logo/' . $dealer_array[0]->dealer_id . '" style="margin-top:10px;" class="delete_confirm delete_link">Delete Logo</a>';
		}
	?>
    <br /><br />
    
    <div class="content_divider"> </div>
    <h3>Contact Request</h3>
    <p class="instruction">Customer information requests from the "contact us" form on the homepage and the "contact us" page will e-mailed to you. Enter the e-mail addresses of employees who should receive these e-mails.</p>
    
    <p class="instruction" style="margin-top:15px;"><b>NOTE:</b> <i>If "Primary Contact Form Recipient" is left blank, e-mails will be sent to the main e-mail address associated with your account</i></p>
    
    <label for="primary_email">Primary Contact Form Recipient</label><br />
    <input type="text" class="input_text" name="primary_email" value="<?php echo set_value('primary_email', $dealer_array[0]->primary_email); ?>" /><br />
    <div class="error_message"><?php echo error_message('primary_email'); ?> </div><br /><br />
    
    <label for="cc_email">Contact form CC: Recipients (separate with commas)</label><br />
    <textarea name="cc_email" class="textarea_text"><?php echo set_value('cc_email', $dealer_array[0]->cc_email); ?></textarea>
    <div class="error_message"><?php echo error_message('cc_email'); ?> </div><br /><br />

    
    <div class="content_divider"> </div>
    <h3>Credentials/Affiliations</h3>
    <label for="name">Dealer Credentials</label><br />
    <textarea name="credentials" class="textarea_text"><?php echo set_value('credentials', $dealer_array[0]->credentials); ?></textarea>
    <div class="error_message"><?php echo error_message('credentials'); ?> </div><br /><br />


    <div class="form_clear"> </div>
    <div id="form_submit_options">
        <a href="/dealer-admin/account"><img src="/_assets/images/admin/default/buttons/cancel_btn.gif" border="0" alt="Cancel" /></a>
        <div id="submit_options">
            <input type="image" src="/_assets/images/admin/default/buttons/update_profile_btn.gif" />
        </div>
        <div class="clear"> </div>
    </div>
    <div class="form_clear"> </div>
</form>